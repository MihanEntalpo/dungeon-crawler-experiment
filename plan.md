...
* Сохранения хранятся в localForage по ключам `map_v1` и `entities_v1`
...
##### Анимация, класс Animation - все анимации лежат в глобальном массиве all_animations
...
* Прозрачность 
* Угол поворота
* Может принимать кастомную функцию рендера (для сущностей, которые рисуются вручную)
* Размер отрисовки может переопределяться полем сущности `renderSize`
* Для сложных мешей поддерживаются флаги рендера (`skipAutoNormalFlip`, `cullBackfaces`)
* PolyHedronDrawer — наследник Drawer для отрисовки многогранников сверху с простым освещением
* Данные мешей многогранников вынесены в реестр `js/meshes.js`, конкретные многогранники лежат в `js/meshes/polyhedra/*.js`
...
##### Мир, класс World:
* Содержит карту (GameMap), туман войны, списки сущностей и ссылку на EntityManager
* Отвечает за обновление мира: update(dt)
* Отвечает за рендер: render(ctx, camX, camY, dpr)
* Предоставляет методы спавна/удаления сущностей
* Содержит HUD как отдельный слой (рисуется поверх тумана)

##### Менеджер сущностей, класс EntityManager:
* Хранит все сущности мира (по спискам и/или по типам)
* Отвечает за добавление/удаление/поиск сущностей
* Содержит проходы updateEntities(dt) и renderEntities(...)
* Может содержать простую spatial-структуру (в будущем)
* Рендер идёт через активный Drawer сущности
...
##### Ввод и камера
* InputManager поддерживает клавиатуру/мышь и touch-управление (двойной джойстик).
* На touch-устройствах атака активна при удержании правого стика.
* Camera в мобильном режиме смещает фокус вперёд по направлению атаки.

##### Слои UI
* Главное меню: «Новая игра» / «Загрузить».
* Пауза: при потере фокуса или Esc, с кнопкой «Продолжить».
